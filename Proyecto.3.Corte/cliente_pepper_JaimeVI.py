# cliente_pepper_Jaime_Final_v6_vocab_port.py
# -*- coding: utf-8 -*-
...
# !!!!! ATENCIÓN AQUÍ: CAMBIA ESTE PUERTO AL PUERTO REAL DE TU SERVIDOR FLASK !!!!!
SERVER_PORT = 9559 # Ejemplo. Podría ser 8000, 8080, u otro. NO 9559 normalmente.
# !!!!! -------------------------------------------------------------------- !!!!!
...
def enviar_al_servidor_flask(mensaje_concatenado):
    print_u(u"[Cliente Pepper] Enviando a Flask ({}:{}): '{}'", SERVER_IP, SERVER_PORT, mensaje_concatenado)
    try:
        conn = httplib.HTTPConnection(SERVER_IP, SERVER_PORT, timeout=15) 
        ...
        conn.close()
        respuesta_json_str = respuesta_json_bytes.decode('utf-8')
        respuesta_data = json.loads(respuesta_json_str)
...
def on_palabra_detectada_callback(word_confidence_list):
    ...
    palabra_procesada = palabra_raw_unicode.lower()
    ...
    if palabra_procesada == KEYWORD_ENVIAR: # KEYWORD_ENVIAR ya es unicode
        print_u(u"[DEBUG] Palabra clave '{}' detectada con confianza {:.2f}", KEYWORD_ENVIAR, confianza)
        if buffer_palabras:
            mensaje_a_enviar = u' '.join(buffer_palabras) 
            buffer_palabras = [] 
            ...
            respuesta_del_servidor = enviar_al_servidor_flask(mensaje_a_enviar)
            print_u(u"[Pepper Log] Respuesta recibida del servidor: '{}'", respuesta_del_servidor)
            safe_say(respuesta_del_servidor)
        else:
    ...
    elif palabra_procesada in VOCABULARIO: # Ahora es comparación unicode in [unicode, unicode, ...]
...
def main():
    ...
    try:
        ...
        if KEYWORD_ENVIAR in VOCABULARIO: # Esto ahora es unicode in [unicode, ...]
