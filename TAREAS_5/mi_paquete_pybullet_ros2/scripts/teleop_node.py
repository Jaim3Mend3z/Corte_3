...
def main(args=None):
    ...
    try:
        ...
        if rclpy.ok():
            if hasattr(teleop_node, '_labels') and teleop_node._labels:
                try: teleop_node.destroy_node()
                except Exception as e:
                    if hasattr(teleop_node, 'get_logger'): teleop_node.get_logger().error(f"Error al destruir el nodo: {e}")
                    else: print(f"Error al destruir el nodo (logger no disponible): {e}")
            rclpy.shutdown()
